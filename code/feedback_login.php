...
<link rel="stylesheet" href="feedbacklogin_css.css">
...
<body>
    ...
    <div id="middlepart">
		...
		foreach ($cursor as $document)
		{
			...
			{
				$bool=$document[$subjects[$i]];
				if($bool=='false')
				{
					$incomp[$incomp_cnt]=$subjects[$i];
					$incomp_cnt+=1;
				}
				else if($bool=='true')
				{
					$comp[$comp_cnt]=$subjects[$i];
					$comp_cnt+=1;
				}
			}
		...
		if(sizeof($incomp)==0)
		{
			$msg="<br>Thank You!<br>You have given feedback for all the subjects";
			echo '<p id="msg">'.$msg.'</p>';
		}
		else
		{
			$msg='<br>Please Complete the feedback for following highlighted subjects';
			echo '<p id="msg">'.$msg.'</p>';
			for($i=0;$i<sizeof($comp);$i++)
			{
				echo '<p id="subjects">'.strtoupper($comp[$i]).'</p>';
			}
			for($i=0;$i<sizeof($incomp);$i++)
			{
				$sub=$incomp[$i];
				
				$linkattr='<br><a href="http://localhost/asm/feedback_page.php?subject='.$incomp[$i].'" id="sub" target="_blank" >'.strtoupper($incomp[$i]).'</a></p>';	
				echo $linkattr;
			}
		}
